...
/**
 * This pass simplifies assignments from havoc'd intermediate variables. It determines intermediate
 * variables based on their usage patterns: `havoc x; y := x` matches when y is not used in other
...
class HavocPromotionAndRangePass(val parseContext: ParseContext) : ProcedurePass {
  ...
  override fun run(builder: XcfaProcedureBuilder): XcfaProcedureBuilder {
    ...
    for (edge in edges) {
      var candidates =
        (edge.label as SequenceLabel)
          .labels
          .mapIndexed { index, it -> Pair(index, it) }
          .filter { it.second is StmtLabel && (it.second as StmtLabel).stmt is HavocStmt<*> }
        ...
        candidates =
          candidates.filter {
            ...
            labels.size == 2 &&
              ...
              labels[1] is StmtLabel &&
              (labels[1] as StmtLabel).stmt is AssignStmt<*> &&
        ...
        if (indices.isNotEmpty()) {
          ...
          for ((index, label) in edge.label.labels.withIndex()) {
            if (indices.size <= offset || index < indices[offset]) newLabels.add(label)
              ...
              val varDecl =
                ((edge.label.labels[index + 1] as StmtLabel).stmt as AssignStmt<*>).varDecl
              //                            val type =
              // CComplexType.getType(((edge.label.labels[index + 1] as StmtLabel).stmt as
              // AssignStmt<*>).expr, parseContext)
